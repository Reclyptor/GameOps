...
| **Backups** | `BACKUP_CRON` → save → archive of the adapter's paths → **read back and verified** → atomic rename → prune older than `BACKUP_RETAIN_DAYS`. Also `gameops backup` by hand; `gameops backup list` and `gameops backup verify latest` to check on them. |
...
| `SERVER_ADDRESS` | — | Where players connect. Every START message carries it and the join password (when set), so the channel always has what players need |
