...
loop:
  ...
  wait game_ready (READY_TIMEOUT) → set ready flag → notify START
  wait for server exit
  if an update was requested:  game_update_apply → notify UPDATE_POST → loop
                               (apply failed → notify UPDATE_FAILED → loop on the installed version;
                                that target is not retried until a different one appears)
  if a restore was requested:  swap the archive in → notify RESTORE_POST → loop
...
nothing. Crashes are *not* retried inside the container; that is the orchestrator's job, and hiding
...
→ wait for a running backup to finish → exit 0. Set the orchestrator's grace period above
`STOP_TIMEOUT`.

### 4.3 Update (`gameops update`, scheduled)
1. Take the job lock (shared with backups). Skip if held.
...
5. If `BACKUP_ON_UPDATE=true` → run a backup cycle.
...
wrong time.
1. Take the lock. 2. `notify BACKUP_PRE`. 3. `game_backup_begin`. 4. `game_save` (if supported).
...
1. Take the lock. 2. Resolve the archive (`latest`, a name in `BACKUP_DIR`, or a path) and verify
...
| `GAME_PASSWORD` | — | Join password, where the game has one. Env only. |
| `SERVER_ADDRESS` | — | How players reach the server (`host:port`, or any text). Only used in notifications, as `server_address`. |
...
| `DISCORD_SILENT_EVENTS` | `BACKUP_PRE,BACKUP_POST` | Events always posted silently, whatever the global flag says. |
...
`file_path`, `backup_kind` (`Scheduled`, `Manual`, `Pre-update`, `Pre-restore`), `warn_minutes`, `restart_note`, `reason`, `server_address` (from `SERVER_ADDRESS`) and `game_password`
(from `GAME_PASSWORD`), plus any `key=value` pairs
passed to `notify`. **A START message always carries the connection details:** when `SERVER_ADDRESS`
or a join password is set and the message text does not place `server_address` / `game_password`
