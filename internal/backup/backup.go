...
// Failed reports a backup of the given kind that did not happen: logged,
// counted and posted. Every way a backup can fail goes through here, so none
// of them is ever silent.
func (b *Backup) Failed(kind string, err error) error {
	logx.Errorf("%s backup failed: %v", strings.ToLower(kind), err)
	b.bump(state.BackupFailuresTotal)
	b.Nt.Send("BACKUP_FAILED", "backup_kind="+kind, "reason="+err.Error())
	return err
}

// Run performs one cycle of the given kind. Callers hold the job lock.
func (b *Backup) Run(kind string) error {
	if err := config.RequireWritable(b.Cfg.BackupDir, "BACKUP_DIR"); err != nil {
		return b.Failed(kind, err)
	}
	paths, _, err := b.Ad.Lines("game_backup_paths")
	if err != nil {
		return b.Failed(kind, fmt.Errorf("game_backup_paths: %w", err))
	}
	var present []string
	for _, p := range paths {
	...
	if len(present) == 0 {
		return b.Failed(kind, fmt.Errorf("no backup paths exist under %s", b.Cfg.DataDir))
	}
	...
	if err != nil {
		os.Remove(tmp)
		return b.Failed(kind, fmt.Errorf("archive failed: %w", err))
	}
	// An archive counts only once it has been read back in full.
	rep, err := Verify(tmp, present)
	if err != nil {
		os.Remove(tmp)
		return b.Failed(kind, fmt.Errorf("verification failed: %w", err))
	}
	if err := os.Rename(tmp, final); err != nil {
		os.Remove(tmp)
		return b.Failed(kind, fmt.Errorf("could not move the verified archive into place: %w", err))
	}
