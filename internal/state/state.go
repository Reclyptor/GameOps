...
// ErrLocked is returned by WithLock when another job holds the lock.
var ErrLocked = errors.New("lock held by another job")

// WithLock runs fn under the shared job lock, returning ErrLocked without
// running it when the lock is held: overlapping backups and updates are
// skipped, not queued.
